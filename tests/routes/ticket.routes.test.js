...
describe("testing the Update ticket feature", async ()=>{

    const apiEndpoint = "/crm/api/v1/tickets/1";

    const token = jwt.sign({ userId: 1 }, config.secret, {
        expiresIn: 600
    });

    jest.spyOn(client, 'post').mockImplementation((url, args, cb) => cb("Test", null));

    it("should be able to successfully update the ticket", ()=>{
